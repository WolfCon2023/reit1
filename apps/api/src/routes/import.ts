...
import { ImportBatch, Site } from "../models/index.js";
...
import { generateTemplate } from "../services/importTemplate.js";
...
router.get("/template", requireAuth, requirePermission(PERMISSIONS.IMPORT_TEMPLATE_DOWNLOAD), async (_req, res) => {
  const buffer = await generateTemplate();
  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  res.setHeader("Content-Disposition", "attachment; filename=REIT_Site_Import_Template.xlsx");
  res.send(buffer);
});
...
router.post("/commit", requireAuth, requirePermission(PERMISSIONS.IMPORT_RUN), async (req, res) => {
  ...
  for (const row of validRows) {
    ...
    const existing = await Site.findOne({ siteId, isDeleted: { $ne: true } });
