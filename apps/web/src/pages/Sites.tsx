...
export function Sites() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["sites", page, search, state, provider, structureType, providerResident],
    queryFn: () => {
      ...
      return api<SitesRes>(`/api/sites?${params}`);
    },
  });
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
              ...
              <select
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                value={structureType}
                onChange={(e) => { setStructureType(e.target.value); setPage(1); }}
              >
              ...
              <select
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                value={providerResident}
                onChange={(e) => { setProviderResident(e.target.value); setPage(1); }}
              >
                <option value="">Provider resident</option>
                {PROVIDER_RESIDENT_OPTIONS.map((s) => (
                  <option key={s} value={s}>{s}</option>
                ))}
              </select>
                  ...
                  <tbody>
                    {items.map((site) => (
                      <tr key={site._id} className="border-b hover:bg-muted/50">
                        <td className="p-2">
                          <Link to={`/sites/${site._id}`} className="text-primary hover:underline">
                            {site.siteId}
                          </Link>
                        </td>
